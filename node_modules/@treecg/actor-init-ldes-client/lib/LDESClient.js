...
class LDESClient extends bus_init_1.ActorInit {
    ...
    async run(action) {
        ...
        if (!args["_"].length) {
            return { stderr: require('streamify-string')(new Error(LDESClient.HELP_MESSAGE)) };
        }
        const pollingInterval = args.pollingInterval ? parseInt(args.pollingInterval) : this.pollingInterval;
        const mimeType = args.mimeType ? args.mimeType : this.mimeType;
        const options = {
            pollingInterval,
            mimeType
        };
        if (args.context && (0, fs_1.existsSync)(args.context)) {
            options.jsonLdContext = JSON.parse((0, fs_1.readFileSync)(args.context, 'utf8'));
        }
        if (args.requestHeadersPath && (0, fs_1.existsSync)(args.requestHeadersPath)) {
            options.requestHeaders = JSON.parse((0, fs_1.readFileSync)(args.requestHeadersPath, 'utf8'));
        }
    ...
    createReadStream(url, options, state = null) {
        ...
        if (!options.jsonLdContext) {
            if (this.jsonLdContextPath && this.jsonLdContextPath.length > 0) {
                options.jsonLdContext = JSON.parse((0, fs_1.readFileSync)(this.jsonLdContextPath, 'utf8'));
            }
        ...
        if (!options.requestHeaders) {
            if (this.requestHeadersPath && this.requestHeadersPath.length > 0) {
                options.requestHeaders = JSON.parse((0, fs_1.readFileSync)(this.requestHeadersPath, 'utf8'));
            }
        ...
        return new EventStream_1.EventStream(url, mediators, options, state);
    }
}
exports.LDESClient = LDESClient;
LDESClient.HELP_MESSAGE = `actor-init-ldes-client syncs event streams
  Usage:
  ...
  `;
//# sourceMappingURL=LDESClient.js.map