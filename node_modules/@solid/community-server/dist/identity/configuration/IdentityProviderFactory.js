...
class IdentityProviderFactory {
    ...
    async initConfig() {
        ...
        config.jwks = await this.generateJwks();
        config.cookies = {
            ...config.cookies,
    ...
    async generateJwks() {
        ...
        const jwks = await this.storage.get(JWKS_KEY);
        if (jwks) {
            return jwks;
        }
        // If they are not, generate and save them
        const { privateKey } = await (0, jose_1.generateKeyPair)(this.jwtAlg);
    ...
    configureClaims(config) {
        ...
        config.extraTokenClaims = async (ctx, token) => this.isAccessToken(token) ?
    ...
    configureRoutes(config) {
        ...
        config.interactions = {
            url: async (ctx, oidcInteraction) => {
                const operation = {
                    method: ctx.method,
                    target: { path: ctx.request.href },
                    preferences: {},
                    body: new BasicRepresentation_1.BasicRepresentation(),
                };
                ...
                throw new InternalServerError_1.InternalServerError('Could not correctly redirect for the given interaction.');
            },
        };
        config.routes = {
    ...
    configureErrors(config) {
        config.renderError = async (ctx, out, error) => {
            ...
            const result = await this.errorHandler.handleSafe({ error, request: (0, GuardedStream_1.guardStream)(ctx.req) });
