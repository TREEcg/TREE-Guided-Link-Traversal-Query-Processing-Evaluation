...
class RegistrationManager {
    ...
    validateInput(input, allowRoot = false) {
        ...
        (0, assert_1.default)(trimmedEmail && emailRegex.test(trimmedEmail), 'Please enter a valid e-mail address.');
        ...
        (0, assert_1.default)(validated.register || validated.createPod, 'Please register for a WebID or create a Pod.');
        (0, assert_1.default)(allowRoot || !validated.rootPod, 'Creating a root pod is not supported.');
        // Parse WebID
        if (!validated.createWebId) {
            const trimmedWebId = this.trimString(webId);
            (0, assert_1.default)(trimmedWebId && (0, HeaderUtil_1.hasScheme)(trimmedWebId, 'http', 'https'), 'Please enter a valid WebID.');
            validated.webId = trimmedWebId;
        }
        // Parse Pod name
        if (validated.createPod && !validated.rootPod) {
            const trimmedPodName = this.trimString(podName);
            (0, assert_1.default)(trimmedPodName && trimmedPodName.length > 0, 'Please specify a Pod name.');
    ...
    async register(input, allowRoot = false) {
        ...
        if (input.createWebId) {
            input.webId = (0, PathUtil_1.joinUrl)(podBaseUrl.path, this.webIdSuffix);
        }
        else {
